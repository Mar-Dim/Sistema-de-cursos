import { CompletedCondition } from "./conditions/completed-condition";
import { ScoreCondition } from "./conditions/score-condition";
import { DecisionTreeNode } from "./decision-tree-node";


export function buildUnlockTree(requiredScore: number, lessonId: number) {
  return new DecisionTreeNode({
    ...
    right: new DecisionTreeNode({
      condition: new ScoreCondition(requiredScore),
